...
def test_parse_binary_op() -> None:
    ...
    assert parse(binary_operation) == ast.Block(None, [], ast.BinaryOp(L,left=ast.Literal(L,2), op="+", right=ast.Literal(L,5)))
...
def test_parse_binary_identifier() -> None:
    ...
    assert parse(binary_operation) == ast.Block(None, [], ast.BinaryOp(L,left=ast.Literal(L,2), op="+", right=ast.Identifier(L,"a")))
...
def test_parse_three_terms() -> None:
    ...
    assert parse(binary_operation) == ast.Block(None, [], ast.BinaryOp(L,left=ast.BinaryOp(L,left=ast.Literal(L,2), op="+", right=ast.Literal(L,5)), op="+", right=ast.Identifier(L,"a")))
...
def test_parse_if_else_clause() -> None:
    ...
    assert parse(if_clause) == ast.Block(None, [], ast.IfExpression(L,ast.Identifier(L,"a"), ast.BinaryOp(L,left=ast.Literal(L,5), op="+", right=ast.Literal(L,5)), ast.BinaryOp(L,left=ast.Literal(L,6), op="+", right=ast.Literal(L,6))))
...
def test_parse_if_without_else() -> None:
    if_clause = [Token(loc=Source_location("if a then 5 + 5 else 6 + 6", 1, 1), type="identifier", text="if"),
                 ...
                 Token(loc=Source_location("if a then 5 + 5 else 6 + 6", 1, 13), type="operator", text="+"),
                 Token(loc=Source_location("if a then 5 + 5 else 6 + 6", 1, 15), type="int_literal", text="5")]
    assert parse(if_clause) == ast.Block(None, [], ast.IfExpression(L,ast.Identifier(L,"a"), ast.BinaryOp(L,left=ast.Literal(L,5), op="+", right=ast.Literal(L,5)), None))
...
def test_parse_if_as_expression() -> None:
    ...
    assert parse(binary_operation) == ast.Block(None, [], ast.BinaryOp(L,ast.Literal(L,1), "+", ast.IfExpression(L,ast.Boolean_literal(L,"true"), ast.Literal(L,2), ast.Literal(L,3))))
...
def test_nested_if() -> None:
    ...
    assert parse(if_clause) == ast.Block(None, [], ast.IfExpression(L,ast.Identifier(L,"a"), ast.IfExpression(L,ast.Boolean_literal(L,"true"), ast.Literal(L,2), ast.Literal(L,3)), None))
...
def test_function_call() -> None:
    ...
    assert parse(function_call) == ast.Block(None, [], ast.Function(L,ast.Identifier(L,"mrFunc"), [ast.Literal(L,4), ast.Identifier(L,"a")]))
...
def test_parenthesis() -> None:
    ...
    assert parse(parenthesis) == ast.Block(None, [], ast.BinaryOp(L,ast.Literal(L,4), "*", ast.BinaryOp(L,ast.Literal(L,4), "-", ast.Literal(L,6))))
...
def test_variable_declaration_top() -> None:
    ...
    assert parse(declaration) == ast.Block(None, [], ast.Declaration(L,variable=ast.Identifier(L,"x"), value=ast.Literal(L,5), typed=None))
...
def test_variable_declaration_block() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[], ast.Declaration(L,variable=ast.Identifier(L,"x"), value=ast.Literal(L,5), typed=None)))
...
def test_block_result() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.Declaration(L,ast.Identifier(L,"x"), ast.Literal(L,5), None)], ast.Identifier(L,"x")))
...
def test_block_no_result() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.Declaration(L,ast.Identifier(L,"x"), ast.Literal(L,5), None), ast.Identifier(L,"x")], ast.Literal(None, None)))
...
def test_block_automatic_semicolons() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.Block(L,[], ast.Declaration(L,ast.Identifier(L,"x"), ast.Literal(L,4), None)), ast.Block(L,[], ast.Identifier(L,"a"))], ast.Block(L,[], ast.Declaration(L,ast.Identifier(L,"x"), ast.Literal(L,5), None))))
...
def test_block_automatic_semicolons_1() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.Block(L,[], ast.Identifier(L,"a"))], ast.Block(L,[], ast.Identifier(L,"b"))))
...
def test_block_automatic_semicolons_3() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.IfExpression(L,ast.Boolean_literal(L,"true"), ast.Block(L,[], ast.Identifier(L,"a")), None)], ast.Identifier(L,"b")))
...
def test_block_automatic_semicolons_6() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.IfExpression(L,ast.Boolean_literal(L,"true"), ast.Block(L,[], ast.Identifier(L,"a")), None), ast.Identifier(L,"b")], ast.Identifier(L,"c")))
...
def test_block_automatic_semicolons_7() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.Block(L,[ast.IfExpression(L,ast.Boolean_literal(L,"true"), ast.Block(L,[], ast.Identifier(L,"a")), ast.Block(L,[], ast.Identifier(L,"b")))], ast.Identifier(L,"c")))
...
def test_block_automatic_semicolons_8() -> None:
    ...
    assert parse(block) == ast.Block(None, [], ast.BinaryOp(L,ast.Identifier(L,"x"), "=", ast.Block(L,[ast.Block(L,[], ast.Function(L,ast.Identifier(L,"f"), [ast.Identifier(L,"a")]))], ast.Block(L,[], ast.Identifier(L,"b")))))
...
def test_while_loop() -> None:
    ...
    assert parse(loop) == ast.Block(None, [], ast.While_loop(L, ast.Boolean_literal(L, "true"), ast.BinaryOp(L, ast.Identifier(L, "a"), "+", ast.Literal(L, 1))))
...
def test_typed_variable_dec() -> None:
    ...
    assert parse(declaration) == ast.Block(None, [], ast.Declaration(L, ast.Identifier(L, "a"), ast.Boolean_literal(L, "true"), Bool))
...
def test_multiple_top_level() -> None:
    ...
    assert parse(code) == ast.Block(None, [ast.Declaration(L, ast.Identifier(L, "a"), ast.Literal(L, 1), None)], ast.Function(L, ast.Identifier(L, "print"), [ast.BinaryOp(L, ast.BinaryOp(L, ast.Identifier(L, "a"), "+", ast.Literal(L, 2)), "==", ast.Literal(L, 3))]))